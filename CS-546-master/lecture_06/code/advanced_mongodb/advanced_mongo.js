...
module.exports = {
    ...
    const movieCollection = await movie.getAllMovie
    // more simple stuff
    getMovie: async (id) => {
        ...
        const searchMovie = await movieCollection.find
        if (!movie) {
    ...
    searchByJavaScriptQuery: async (keyword) => {
        ...
        return await movieCollection
            .find({
                $where:
                    "this.title.toLowerCase().indexOf('" + keyword + "') >= 0",
            })
            .toArray();
    },
    ...
    updateRatingToMinValue: async function (id, newRating) {
        ...
        if (newRating === undefined)
            return Promise.reject("no rating provided");
        const movieCollection = await movies();
    ...
    findByCast: async function (name) {
        if (!name)
            return Promise.reject(
                "You must provide a name for the cast member"
            );
        const movieCollection = await movies();
    ...
    removeCastMember: async function (id, memberToRemove) {
        if (id === undefined) return Promise.reject("No id provided");
        if (!memberToRemove)
            return Promise.reject("No memberToRemove provided");
        const movieCollection = await movies();
